...
if (preg_match('/INTENT:\s*RECOMENDAR_POR_PRESUPUESTO\s+(\d+)/i', $reply, $match)) {
    ...
    $consulta = "SELECT * FROM paquetes WHERE precio_total <= $presupuesto AND cupos_disponibles > 0";
    $resultado = mysqli_query($enlace, $consulta);
    if (mysqli_num_rows($resultado) > 0) {
        $paquetes = [];
        while ($row = mysqli_fetch_assoc($resultado)) {
            $paquetes[] = "ID: {$row['id']} | {$row['nombre']} - {$row['precio_total']}$ - Salida: {$row['fecha_salida']}";
        }
